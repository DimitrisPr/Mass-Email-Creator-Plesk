...
def main():
    ...
    # async calls or threading would be the optimal solutions for production apps. 
...
def create_email():
    ...
    email_field = \
        driver.find_element_by_id('general-generalSection-name')
    ...
    password_field = \
        driver.find_element_by_id('general-generalSection-password')
    password_field.send_keys(email_password)

    password_confirmation_field = \
        driver.find_element_by_id('general-generalSection-passwordConfirmation')
    password_confirmation_field.send_keys(email_password)

    password_confirmation_field.submit()

    time.sleep(2)  # Should be changed with selenium's ImplicitWait function
